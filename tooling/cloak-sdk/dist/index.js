...
import { blake3 as blake33 } from "@noble/hashes/blake3.js";
...
function deriveSpendKey(masterSeed) {
  ...
  const sk_spend = blake32(preimage);
  const pk_spend = blake32(sk_spend);
  return {
    sk_spend,
    pk_spend,
...
function deriveViewKey(sk_spend) {
  ...
  const vk_secret = blake32(preimage);
...
function generateNote(amountLamports, network) {
  ...
  commitmentInput.set(pkSpend, 40);
  const commitmentBytes = blake33(commitmentInput);
  const skSpendHex = Buffer2.from(skSpend).toString("hex");
  const rHex = Buffer2.from(rBytes).toString("hex");
  const commitmentHex = Buffer2.from(commitmentBytes).toString("hex");
  return {
    version: "1.0",
...
function generateNoteFromWallet(amountLamports, keys, network) {
  ...
  const rBytes = new Uint8Array(32);
  crypto.getRandomValues(rBytes);
  const pk_spend = Buffer2.from(keys.spend.pk_spend_hex, "hex");
  const amountBytes = new Uint8Array(8);
  new DataView(amountBytes.buffer).setBigUint64(0, BigInt(amountLamports), true);
  const commitmentInput = new Uint8Array(8 + 32 + 32);
  commitmentInput.set(amountBytes, 0);
  ...
  return {
    version: "2.0",
    amount: amountLamports,
    commitment: Buffer2.from(commitmentBytes).toString("hex"),
    sk_spend: keys.spend.sk_spend_hex,
    r: Buffer2.from(rBytes).toString("hex"),
    timestamp: Date.now(),
    network: actualNetwork
  };
}
...
function validateDepositParams(params) {
  ...
  if (!(params.commitments instanceof PublicKey2)) {
