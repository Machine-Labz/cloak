...
fn test_withdraw_instruction() {
    ...
    let sp1_proof = cloak_proof_extract::extract_groth16_260(&full_proof_bytes)
        .expect("extract groth16 proof from SP1 bundle");
    ...
    let expected_body_len = sp1_proof.len()
        + sp1_public_inputs.len()
        + nullifier.len()
        + 1
    ...
    assert_eq!(
        ...
        expected_body_len,
        "Withdraw instruction data should be {} bytes, got: {}",
        expected_body_len,
        withdraw_instruction_data.len()
    );

    // Validate full instruction data length (body + 1 for discriminant)
    assert_eq!(
        full_instruction_data.len(),
        expected_body_len + 1,
        "Full instruction data should be {} bytes, got: {}",
        expected_body_len + 1,
        full_instruction_data.len()
    );
