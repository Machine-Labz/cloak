...
fn prove_command(
    ...
    let outputs_json = fs::read_to_string(&outputs_path)
        .map_err(|e| anyhow!("Failed to read outputs: {}", e))?;
    ...
    // Generate proof
    let guest_elf = std::fs::read("target/elf-compilation/riscv32im-succinct-zkvm-elf/release/zk-guest-sp1-guest")
        .or_else(|_| std::fs::read("../guest/target/riscv32im-succinct-zkvm-elf/release/zk-guest-sp1-guest"))
