...
/// For production use, consider implementing client-side proof generation.
pub async fn generate_proof(
    ...
    let client_ip = "unknown"; // In production, extract from request headers
    ...
    match sp1_generate_proof(&request.private_inputs, &request.public_inputs, &request.outputs) {
        Ok(proof_result) => {
            tracing::info!("SP1 proof generation succeeded");
            tracing::info!("Proof size: {} bytes", proof_result.proof_bytes.len());
            tracing::info!("Public inputs size: {} bytes", proof_result.public_inputs.len());
