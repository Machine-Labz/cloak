...
async fn cloudwatch_log_sender(
    ...
    while let Some(message) = rx.recv().await {
        ...
        // Send in batches of 10 or after 1 second
        if batch.len() >= 10 {
...
pub async fn init_logging_with_cloudwatch(
    ...
    let log_stream_name = format!("relay-{}", machine_id);
    ...
    // Create env filter for console output
    let env_filter = EnvFilter::from_default_env();

    // Initialize subscriber with console logging
