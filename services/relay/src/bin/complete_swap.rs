...
async fn main() -> Result<()> {
    ...
    if args.len() < 2 {
        ...
        eprintln!();
        eprintln!("Example:");
        eprintln!("  {} 650dbce1b64abc09c9ede2421c1c9102ea161db012be12c61f7951b5bbed81c0", args[0]);
        std::process::exit(1);
    }
    ...
    // Load relay keypair
    let relay_keypair = read_keypair_file(&relay_keypair_path)
        .map_err(|e| anyhow::anyhow!("Failed to read keypair from {}: {}", relay_keypair_path, e))?;
    let relay_pubkey = relay_keypair.pubkey();
    println!("✅ Relay: {}", relay_pubkey);
    
    // Decode nullifier
    let nullifier_bytes = hex::decode(nullifier_hex)
        .map_err(|e| anyhow::anyhow!("Invalid nullifier hex: {}", e))?;
    ...
    let (swap_state_pda, _bump) = Pubkey::find_program_address(
        &[b"swap_state", &nullifier],
        &program_id,
    );
    ...
    let swap_state_account = client.get_account(&swap_state_pda)
        .map_err(|e| anyhow::anyhow!("SwapState PDA not found - swap may already be completed: {}", e))?;
    
    if swap_state_account.data.len() != 121 {
        anyhow::bail!("Invalid SwapState account size: {}", swap_state_account.data.len());
    }
    ...
    println!("   SOL Amount: {} ({} SOL)", sol_amount, sol_amount as f64 / 1e9);
    ...
    println!("   1. Use the relay's SOL to swap {} SOL → {} on Jupiter", sol_amount as f64 / 1e9, output_mint);
    println!("   2. Send the output tokens to: {}", recipient_ata);
    println!("   3. Ensure at least {} tokens are received", min_output_amount);
    println!();
    ...
    let exec_sig = client.send_and_confirm_transaction(&tx)
    ...
    match client.get_account(&swap_state_pda) {
        ...
        Err(_) => {
            ...
            println!("   Relay reimbursed: {} lamports", swap_state_account.lamports);
        }
    }
