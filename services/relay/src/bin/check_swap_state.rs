...
fn main() -> anyhow::Result<()> {
    ...
    match client.get_account(&swap_state_pda) {
        Ok(account) => {
            ...
            println!("   Lamports: {} ({} SOL)", account.lamports, account.lamports as f64 / 1e9);
            ...
            if account.data.len() >= 121 {
                ...
                println!("   1. Execute Jupiter swap: {} SOL → {} tokens (to {})", 
                    sol_amount as f64 / 1e9, output_mint, recipient_ata);
                println!("   2. Call ExecuteSwap instruction with nullifier: {}", nullifier_hex);
                println!("   3. This will close the PDA and return {} lamports to relay", account.lamports);
            }
        }
        Err(e) => {
