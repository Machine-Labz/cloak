...
impl JobQueue for RedisJobQueue {
    ...
    async fn dequeue(&self, _timeout: Duration) -> Result<Option<JobMessage>, Error> {
        ...
        if let Some((job_json, _score)) = result.into_iter().next() {
            ...
            let processing_json = serde_json::to_string(&processing_data).map_err(|e| {
                Error::RedisError(format!("Failed to serialize processing data: {}", e))
            })?;
            ...
            let _: () = conn
                ...
                .await
                .map_err(|e| {
                    Error::RedisError(format!("Failed to add to processing queue: {}", e))
                })?;
    ...
    async fn requeue_with_delay(&self, message: JobMessage, delay: Duration) -> Result<(), Error> {
        ...
        let _: () = conn
            .zadd(RETRY_QUEUE_KEY, job_json, retry_at)
            .await
            .map_err(|e| Error::RedisError(format!("Failed to requeue job: {}", e)))?;

        debug!(
            "Job requeued for retry: {} (attempt: {}, delay: {}s)",
    ...
    async fn dead_letter(&self, message: JobMessage, reason: String) -> Result<(), Error> {
        ...
        let dead_letter_json = serde_json::to_string(&dead_letter_data).map_err(|e| {
            Error::RedisError(format!("Failed to serialize dead letter data: {}", e))
        })?;
    ...
    async fn queue_size(&self) -> Result<u64, Error> {
        ...
        let retry_size: u64 = conn
            .zcard(RETRY_QUEUE_KEY)
            .await
            .map_err(|e| Error::RedisError(format!("Failed to get retry queue size: {}", e)))?;
        let processing_size: u64 = conn.zcard(PROCESSING_KEY).await.map_err(|e| {
            Error::RedisError(format!("Failed to get processing queue size: {}", e))
        })?;
    ...
    pub async fn cleanup_processing_queue(&self) -> Result<u32, Error> {
        ...
        for job_json in stale_jobs {
            if let Ok(processing_data) = serde_json::from_str::<serde_json::Value>(&job_json) {
                if let Ok(message) =
                    serde_json::from_value::<JobMessage>(processing_data["job"].clone())
                {
