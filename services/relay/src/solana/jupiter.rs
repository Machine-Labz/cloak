...
impl JupiterService {
    ...
    pub fn new(config: JupiterConfig) -> Result<Self, Error> {
        ...
        let client = Client::builder()
            ...
            .build()
            .map_err(|e| Error::InternalServerError(format!("Failed to create HTTP client: {}", e)))?;
    ...
    pub async fn get_quote(
        ...
        let quote: QuoteResponse = response
            .json()
            .await
            .map_err(|e| Error::InternalServerError(format!("Failed to parse quote response: {}", e)))?;
    ...
    pub async fn swap(
        ...
        let swap_response: SwapResponse = response
            .json()
            .await
            .map_err(|e| Error::InternalServerError(format!("Failed to parse swap response: {}", e)))?;

        // Decode the base64-encoded transaction
        let tx_bytes = base64::decode(&swap_response.swap_transaction)
            .map_err(|e| Error::InternalServerError(format!("Failed to decode swap transaction: {}", e)))?;

        let transaction: Transaction = bincode::deserialize(&tx_bytes)
            .map_err(|e| Error::InternalServerError(format!("Failed to deserialize transaction: {}", e)))?;
    ...
    pub async fn is_swap_acceptable(
        ...
        let impact = self.get_price_impact(input_mint, output_mint, amount).await?;
