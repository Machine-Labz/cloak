...
async fn main() -> anyhow::Result<()> {
    ...
    let admin_keypair = {
        let admin_keypair_data: Vec<u8> = serde_json::from_str(
            ...
            .unwrap_or_else(|_| {
                panic!("Failed to read admin keypair from ~/.config/solana/id.json")
            }),
        )
        .unwrap();
    ...
    // These will be set after deposit to indexer
    let mut leaf_index: u32 = 0;
    let mut nullifier: blake3::Hash = blake3::hash(&[]);
    ...
    let deposit_signature = client.send_and_confirm_transaction(&deposit_tx)?;
    ...
    println!(
        "   ✅ Real deposit transaction successful: {}",
        deposit_signature
    );
    ...
    println!(
        "      - User wallet: {} SOL",
        user_balance_before / SOL_TO_LAMPORTS
    );
    println!(
        "      - Admin wallet: {} SOL",
        admin_balance_before / SOL_TO_LAMPORTS
    );
    println!(
        "      - Pool account: {} SOL",
    ...
    println!("   - Extracted proof size: {} bytes", proof_bytes.len());
    ...
    // Prepare withdraw data - use the same fee calculation as above
    let recipient_amount = amount - fee; // Use the same amount as calculated above
    let num_outputs = 1u8;
    ...
    // Set higher compute unit limit for SP1 proof verification (standard for SP1)
    let compute_unit_ix = ComputeBudgetInstruction::set_compute_unit_limit(275_000);
    ...
    // Simulate transaction first to get compute units
    println!("   🔍 Simulating withdraw transaction to measure compute units...");
    match client.simulate_transaction(&withdraw_tx) {
        Ok(simulation_result) => {
            if let Some(units_consumed) = simulation_result.value.units_consumed {
                println!("   📊 Compute units consumed: {}", units_consumed);
                println!(
                    "   💰 Estimated cost: ~{:.6} SOL",
                    units_consumed as f64 * 1e-9
                );
                if units_consumed > 200_000 {
                    println!("   ⚠️  WARNING: Exceeds 200,000 CU limit!");
                } else {
                    println!("   ✅ Within 200,000 CU limit");
                }
            }
        }
        Err(e) => {
    ...
    println!("   ✅ Test process stopped gracefully");
    Ok(())
}
